...
void sigchld_handler(int sig) {
    ...
    while (waitpid(-1, 0, WNOHANG) > 0);  /** WNOHANG: return immediately if no child has exited.*/
    return;
}
...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (fork() == 0) {
            close(listenfd); /** close child process's listenfd */
            echo(connfd); /** Child serves the client */
            close(connfd); /** Child close the connection with the client */
            exit(0); /** Child exits */
        }
