...
void DisjointSets::dsunion(int i, int j) {
    bool i_had_cycle = query_cycle(i); // if has_cycle[i] is true;
    bool j_had_cycle = query_cycle(j); // if has_cycle[j] is true;
    int root_i = find_leader(i); // root of i
    int root_j = find_leader(j); // root of j
    if (root_i != root_j) { // if root of i is not root of j
        leader[root_i] = root_j; // set j as root of (root of i)
        root_i = root_j; // set root of i equals to root of j
    }
    else { // vertex i and vertex j are in the same disjoint set
    ...
    // TODO: Your work here!
    if(query_cycle(root_i) || query_cycle(root_j)) { // if either one has cycle
        int tmp = leader[i];
        while(tmp > 0) {
            has_cycle[tmp] = true;
            tmp = leader[tmp];
...
void DisjointSets::count_comps(int n) {
    ...
    // TODO: Your work here!
    int hash[MAX_NODES]={0}; // store the distinct leaders
    for(int i=0;i<n;i++) {
        if(leader[i]!=-1) {
            hash[find_leader(i)]++;
        }
